...
export async function POST() {
  ...
  let res = await fetch(`${API_BASE_URL}/api/v1/tasks/auto-plan-day/`, {
    method: 'POST',
  ...
  if (res.status === 401) {
    ...
    if (refresh) {
      ...
      if (r.ok) {
        ...
        res = await fetch(`${API_BASE_URL}/api/v1/tasks/auto-plan-day/`, { method: 'POST', headers: { Authorization: `Bearer ${j.access}` } });
