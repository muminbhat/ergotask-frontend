...
export async function POST(request: Request) {
  ...
  let res = await fetch(`${API_BASE_URL}/api/v1/tasks/nl-create/`, {
    method: 'POST',
  ...
  if (res.status === 401) {
    ...
    if (refresh) {
      ...
      if (r.ok) {
        ...
        res = await fetch(`${API_BASE_URL}/api/v1/tasks/nl-create/`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${j.access}` }, body: JSON.stringify(body) });
