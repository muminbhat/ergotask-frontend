...
export async function GET(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  let res = await fetch(`${API_BASE_URL}/api/v1/tasks/${id}/`, {
  ...
  if (res.status === 401) {
    ...
    if (refresh) {
      ...
      if (r.ok) {
        ...
        next.cookies.set('access_token', j.access, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', path: '/' });
        // retry original
        res = await fetch(`${API_BASE_URL}/api/v1/tasks/${id}/`, {
  ...
  if (res.status === 401) {
    ...
    if (refresh) {
      ...
      if (r.ok) {
        ...
        res = await fetch(`${API_BASE_URL}/api/v1/tasks/${id}/`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${j.access}` },
          body: JSON.stringify(body),
        });
        const data = await res.json();
        const reply = NextResponse.json(data, { status: res.status });
  ...
  if (res.status === 401) {
    ...
    if (refresh) {
      ...
      if (r.ok) {
        const j = await r.json();
        res = await fetch(`${API_BASE_URL}/api/v1/tasks/${id}/`, {
          method: 'DELETE', headers: { Authorization: `Bearer ${j.access}` },
        });
