...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  let res = await fetch(`${API_BASE_URL}/api/v1/tasks/${id}/link-contexts-ai/`, {
    method: 'POST',
  ...
  if (res.status === 401) {
    ...
    if (refresh) {
      ...
      if (r.ok) {
        ...
        res = await fetch(`${API_BASE_URL}/api/v1/tasks/${id}/link-contexts-ai/`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${j.access}` }, body: JSON.stringify(body) });
